...
client.on('ready', () => {
  ...
  const acti = ['Dems 🖖', 'By Sparte', 'dév', 'Commande !edt'];
  const typeacti = ['LISTENING', 'WATCHING', 'PLAYING', 'STREAMING'];
  let i = 0;
  setInterval(() => {
...
client.on('messageCreate', async (message) => {
  ...
  if (!command) {
    const noExist = message.reply("Cette commande n'existe pas !");
    setTimeout(() => {
      noExist.delete();
    }, 5000);
  }
...
client.on('messageCreate', async (message) => {
  if (!message.author.bot) {
    if ((message.channel.name !== 'general') && (message.channel.name !== 'test') && (message.channel.name !== '😼') && (message.channel.name !== 'message')) {
      ...
      await message.channel.send(`${author}:\`\`\`${channel}\n${content} \`\`\``);
      message.delete();
    }
  }
});
